...
class SeedFixturesAdmin {
    ...
    public static function render_page() {
        ...
        echo '<p>' . esc_html__( 'Start with proven AI fix pages for the clearest client walkthrough. These actions refresh the demo pages and clear old scan results.', 'accessibility-auditor' ) . '</p>';
        ...
        echo '<p style="margin-top:0;">' . esc_html__( 'Use these filters only if you want a specific demo. Otherwise just prepare proven AI fix pages and open one in Bricks.', 'accessibility-auditor' ) . '</p>';
    ...
    private static function filter_fixtures( array $fixtures, array $filters ): array {
        return array_values( array_filter( $fixtures, static function( array $fixture ) use ( $filters ): bool {
            ...
            $effective_strategy = self::effective_strategy( $fixture );
            if ( ! empty( $filters['scenario'] ) && $fixture['scenario'] !== $filters['scenario'] ) {
                return false;
            }
            if ( ! empty( $filters['strategy'] ) && $effective_strategy !== $filters['strategy'] ) {
                return false;
            }
    ...
    private static function strategy_options( array $fixtures ): array {
        $options = [];
        foreach ( $fixtures as $fixture ) {
            $strategy = self::effective_strategy( $fixture );
            if ( $strategy === 'flagged' || $strategy === '' ) {
                continue;
            }
    ...
    private static function effective_strategy( array $fixture ): string {
        $strategy = (string) ( $fixture['strategy'] ?? '' );
        if ( $strategy === 'auto-fix' && self::is_proven_ai_fix( $fixture ) ) {
            return 'proven-auto-fix';
        }
